...
export class FlowValidationEngine {
  validateFlow(
    ...
    const hasLoginRequest = requests.some(
      (r) =>
        /\/(login|signin|auth)/i.test(r.request.url) &&
        r.request.method === 'GET'
    );

    const hasAuthSubmission = requests.some(
      (r) =>
        /\/(login|signin|auth)/i.test(r.request.url) &&
    ...
    const criticalCompleteness =
      criticalComponents.filter((c) => c).length / criticalComponents.length;
    completeness = Math.max(completeness, criticalCompleteness * 0.6);

    // Check state transitions
    if (stateTransitions.length > 0) {
      const hasAuthTransition = stateTransitions.some(
        (t) => t.from === 'LOGIN_PAGE' && t.to === 'AUTH_SUBMISSION'
      );

      const hasSessionTransition = stateTransitions.some(
        (t) => t.from === 'AUTH_SUBMISSION' && t.to === 'SESSION_ESTABLISHED'
      );

      const transitionCompleteness =
        (hasAuthTransition ? 0.5 : 0) +
        (hasSessionTransition ? 0.5 : 0);
