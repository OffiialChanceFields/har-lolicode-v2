...
export class StateTransitionModeler {
    ...
    for (let i = 0; i < requests.length - 1; i++) {
      ...
      if (fromState !== toState) {
        transitions.push({
          ...
          to: toState,
          trigger: requests[i],
          confidence: this.calculateTransitionConfidence(
            requests[i],
            requests[i + 1]
          )
        });
  ...
  private inferState(entry: HarEntry): string {
    ...
    if (
      /\/(login|signin)/i.test(entry.request.url) &&
  ...
  private getStateName(entry: HarEntry): string {
    ...
    if (
      /\/(login|signin|authenticate)/i.test(path) &&
      entry.request.method === 'POST'
    ) {
  ...
  private calculateTransitionConfidence(
    ...
    const refererHeader = toEntry.request.headers.find(
      (h) =>
